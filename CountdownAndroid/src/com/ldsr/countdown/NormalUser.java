...
public class NormalUser extends Activity {
	...
	TextView timedisplay,exists,starteddisp,nums,res,ispass,code,playerid;
	EditText diffi,time,name,dist,password,id;
		...
		private class getgamestask extends AsyncTask<String, Integer,String[][]>{
				...
				BufferedReader br = null;
	 		      ...
	 		      try { 
	 					...
	 					for(int j = 0; j< Jresults.length(); j++){
	 						...
	 						stringArray[0]= person.getString("name");
	 						stringArray[1]= person.getString("numplayers");
	 						stringArray[2]= person.getString("diff");
			...
			protected void onPostExecute(String[][] results) {
				 ...
				 mo2.clear();
					for(int player = 0; player<results.length;player++){
						String diff = "";
						if(results[player][2].contentEquals("0")){
							diff="Easy";
						}else if(results[player][2].contentEquals("1")){
							diff="Med";
						}else if(results[player][2].contentEquals("2")){
							diff="Hard";
						}else if(results[player][2].contentEquals("3")){
							diff="Extreme";
						}else if(results[player][2].contentEquals("4")){
							diff="All";
						}else
						
					 mo2.add(new Myobject(results[player][0], diff, results[player][1]));
